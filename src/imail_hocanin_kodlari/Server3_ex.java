...
// public class Server3_ex {
//     private static final int PORT = 5003;
//     private static final String SERVER_HOST = "localhost";// Server3's port
...
public class Server3_ex {
    private static final int SERVER3_PORT = 5003; // Server1's port
    private static final String SERVER3_HOST = "localhost"; // Sunucunun gerçek IP adresi
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < 100; i++) {
            aboneler.getGirisYapanlar().add(false);
        }

        ServerSocket serverSocket = new ServerSocket(SERVER3_PORT, 50, InetAddress.getByName(SERVER3_HOST));
        System.out.println("Server1 is running on port " + SERVER3_PORT);

        new PingThread("localhost", 5002).start(); // Ping Server2
        new PingThread("localhost", 5001).start(); // Ping Server3
    ...
    private static class ClientHandler extends Thread {
        ...
        private String girisYap(int istemciNo) {
            if (istemciNo < aboneler.getGirisYapanlar().size() && aboneler.getAboneDurumu().get(istemciNo)) {
